...
function draw() {
    ...
    // Background & Road logic
    ctx.fillStyle = "#151518"; ctx.fillRect(0, 0, 600, 400); // Sky
    ctx.fillStyle = "#0d3d0d"; ctx.fillRect(0, 200, 600, 200); // Grass
    ...
    if (gameState.isRacing) {
        ...
        gameState.score += Math.floor(gameState.speed / 50);
        document.getElementById('score').innerText = gameState.score;
        document.getElementById('speed').innerText = Math.floor(gameState.speed);
        if (gameState.speed < 220) gameState.speed += 0.1;
    }
